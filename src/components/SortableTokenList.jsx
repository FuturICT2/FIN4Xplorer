...
function SortableTokenList(props, context) {
	...
	return (
		<>
			{props.showFilterAndSortOptions && ( // TODO integrate these into translation files
				<>
					<TableIcons>
						<FontAwesomeIcon
							icon={faFilter}
			...
			<Table
				headers={[t('tokens-list.token-name'), t('tokens-list.total-supply'), t('tokens-list.actions')]}
				colWidths={[74, 12, 14]}>
							...
							data={{
									...
									<>
										...
										{token.isOPAT && (
											<Link to="/governance/listing">
												<FontAwesomeIcon title={t('tokens-list.token-is-opat')} icon={faStar} style={styles.iconOPAT} />
											</Link>
										)}
									</>
								),
								...
								actions: (
									<small style={{ color: 'blue', textDecoration: 'underline' }}>
										<Link to={'/token/view/' + token.symbol}>{t('tokens-list.view-button')}</Link>
										<br />
										<Link to={'/claim/' + token.symbol}>{t('tokens-list.claim-button')}</Link>
									</small>
								)
							}}
						/>
